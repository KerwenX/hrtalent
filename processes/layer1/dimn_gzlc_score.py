...
def calculate_in_bank_working_experience_score(session):
    ...
    # 计算每个员工在每个二级序列下的工作年限
    df_working_merge_base_seq['岗位工作时间'] = df_working_merge_base_seq.apply(lambda x: (datetime.datetime.strptime(x['a8662'], '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(x['a8661'], '%Y-%m-%d %H:%M:%S')).days / 365, axis=1)
    ...
    df_base_merge_seq_now = df_base_merge_seq_now[['a0188', '当前序列得分']].drop_duplicates()
